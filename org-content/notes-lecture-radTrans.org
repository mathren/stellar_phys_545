#+title: ASTR545 - Radiative transfer
...
material with the interstellar material! In other words, the stellar
atmosphere is not /necessarily/ in hydro-/static/ equilibrium!
...
density $\rho$ on the rx.h.s. expresses that the more matter there is (per
...
that is the black body flux $F$ is obtained by integrating the
specific intensity over frequency $\nu$ solid angle $\Omega$. Note the factor
$\cos(\theta)$ that arises because $I_{\nu}$ is a vector and we only want the
component normal to the surface element $dA$.
...
\end{equation}
#+end_latex
where we introduce the definition of specific (per unit frequency $\nu$)
optical depth $d\tau_{\nu} = \kappa_{\nu}\rho ds$. This variable is useful because it gives
the scale-length of the problem as depending on $\kappa_{\nu}\rho = 1/\ell_{\gamma,\nu}$ with
...
The third approximation (after the geometry being plane parallel and
the steady state) of the Eddington atmosphere is that we assume a
"gray" radiative transfer, meaning the opacity is /independent of
frequency/ $\kappa_{\nu}\rightarrow\kappa$, thus $\tau_{\nu}\rightarrow\tau$. We also neglect the frequency
dependence of the source term $S_{\nu}$. With these hypotheses we can now
integrate this in $d\nu$ from 0 to +\infty and obtain:
...
Substituting for S in the solution for I (Eq. \ref{eq:I_Edd}) we get:
#+begin_latex
\begin{equation}
...
\end{equation}
#+end_latex
We obtained this constant imposing the flux to come from $\tau=0 \Rightarrow \kappa =
0$, so from the layer after which there is nothing impeding the
photons anymore (*N.B.:* the only other option is $\rho=0$, so there is
nothing, or $ds=0$, so the photons have not moved!). With this
constant, we completely specified the source function $S \equiv S(\tau)$ and
we can obtain $I\equiv I(\tau)$ and use it to calculate the pressure!

* Boundary conditions for the stellar problem
...
temperature of the black body to the outer temperature profile $T(\tau)$
under the approximations for the atmosphere:
1. plane parallel (to make it a one-dimensional problem)
2. static ($\partial_{t} =0$)
3. gray (i.e., independent on frequency \nu)
...
mass $M$ and composition, with boundary conditions:

#+begin_latex
\begin{equation}
P(\tau=2/3) =  \frac{2}{3}\frac{GM}{R^{2}\kappa}\ \ ,\\
T(\tau=2/3) = T_{\rm eff}\ \ , \\
\end{equation}
#+end_latex
...
To completely specify the problem, we need two other boundary
conditions for the structure equations, which are readily obtained
requiring the central density $\rho\sim m/r^{3}$ and energy generation rate $\epsilon_{\rm nuc}-\varepsilon_{\nu}+\varepsilon_{\rm grav}$ to be finite as we go to the center of the star.
#+begin_latex
\begin{equation}
m(r=0)   = 0 \ \ ,\\
L(r=0)   = 0 \ \ ,
\end{equation}
#+end_latex
and finally we need to know the initial composition of the star at the
beginning $X_{i}(t=0)$. This completely specifies the boundary conditions
for the structure at every snapshot in time, and the initial
conditions for the composition equations. Note how the boundary
conditions for the stellar structure equations are specified two at
the photosphere ($\tau=2/3$) and two in the center ($r=0$).

* Saha equation
In general, for the gas in the stellar atmosphere, we need to
calculate the (specific) opacity $\kappa_{\nu}$, which depends sensitively on
the populations of electrons at each energy level of each ion present.
In the simplest case, we can assume that LTE holds also in the stellar
atmosphere, therefore, the rate at which atoms are ionized $I$ matches
the rate at which there are recombinations $R$ (principle of detailed
balance). Therefore:
...
($\omega\ge0.7\omega_\mathrm{crit}$ where $\omega_{\rm crit}= \sqrt{(1-L/L_{\rm Edd})GM/R^{3}^{}$
is the rotation rate at which self-gravity is balanced by radiation
pressure + centrifugal forces) which shed a "decretion disk": the
emission lines are not from the star directly, but from the disk of
the star! A clear indication of the presence of the disk is the
"double peaked" morphology of the H\alpha emission:
...
#+CAPTION: Spectrum of WR137c normalized, from [[https://ui.adsabs.harvard.edu/abs/2012A%26A...540A.144S/abstract][Sander et al. 2012]].
#+ATTR_HTML: :width 100%
[[./images/WR137.png]]
...
# "idealized" surface where $T=T_{\rm eff}$ which corresponds to the
# location outside of which LTE is not a good assumption anymore, the
# radiation field is /not/ isotropic, and the problem becomes the
# calculation of a stellar /atmosphere/.
