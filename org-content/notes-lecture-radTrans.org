...
\end{equation}
#+end_latex
where the last two just come from requiring the density $\rho$ and the
energy generation rate $\epsilon_{\rm nuc}-\varepsilon_{\nu}+\varepsilon_{\rm grav}$ to be finite as we go
