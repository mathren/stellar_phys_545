...
T(\tau=2/3) = T_{\rm eff}\ \ , \\
\{X_{i}_{}\}(t=0) \ \mathrm{known} \ \ ,\\
m(r=0)   = 0 \ \ ,\\
L(r=0)   = 0 \ \ ,
\end{equation}
#+end_latex
where the third just says we know the initial composition, and the
last two just come from requiring the density $\rho$ and the energy
generation rate $\epsilon_{\rm nuc}-\varepsilon_{\nu}+\varepsilon_{\rm grav}$ to be finite as we go to the
center of the star. Note how the boundary conditions for the stellar
structure equations are specified two at the photosphere ($\tau=2/3$) and
two in the center ($r=0$).
