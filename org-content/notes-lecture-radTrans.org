...
beginning $X_{i}_{}(t=0)$. This completely specifies the boundary
conditions for the structure at every snapshot in time, and the
initial conditions for the composition equations. Note how the
boundary conditions for the stellar structure equations are specified
two at the photosphere ($\tau=2/3$) and two in the center ($r=0$).
