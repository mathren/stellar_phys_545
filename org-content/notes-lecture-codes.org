#+Title: ASTR545 - Computational stellar physics
#+author: Mathieu Renzo
#+email: mrenzo@arizona.edu
#+PREVIOUS_PAGE: notes-lecture-radTrans.org
#+NEXT_PAGE: notes-lecture-formation.org
...
Because of this, it is always crucially important to /do tests/: when
performing a simulation of a physical phenomenon, you should always
test that the scientific results you obtain do not depend on the
numerical approximations needed (e.g., how you discretize your
equations in time and space should not matter, which you can verify
doing /resolution tests/). This is often a painful task, but very
...
Consider again the Sun. The free fall timescale is \tau_{ff}\sim1h, and its
thermal timescale is \tau_{KH}\sim1.5\times10^{7} years, and current age is \sim 5\times10^{9}
years \simeq \tau_{nuc}. In 3D we would need to resolve the dynamics of the gas,
with timesteps \Delta t \leq \tau_{ff}. Even considering an equal sign \Delta t=\tau_{ff}
(certainly insufficient to /resolve/ the dynamics of the stellar
plasma), it would take \sim \tau_{Kh} / \tau_{ff} = 10^{10} timesteps to calculate a
thermal timescale and \sim \tau_{nuc} / \tau_{ff} = 10^{12} timesteps for a nuclear
timescale.
...
** Solving strategies
For the /structure/ of the star, we have 4 coupled non-linear ODEs with
4 [[./notes-lecture-radTrans.org::*Boundary conditions for the stellar problem][boundary conditions]] (2 at the center and 2 at the surface), and the
EOS as a closure condition. We also have a set of equations for the
composition with initial values assumed to be known $X_{i}(t=0)$. How can
we solve this coupled non-linear system numerically?
...
\end{equation}
#+end_latex
where $f$ and $x$ are a generic function and variable, and the index
$k$ labels the discretized points.
...
- we use $m$ rather than $r$ as independent coordinate [[./notes-lecture-HSE.org::*Lagrangian and Eulerian coordinates][as expected]] to
  have a Lagrangian description of the stellar fluid;
...
To write this in a symbolic compact form, we define an array of
quantities $\mathcal{X}(t)$ of length equal to the number of mesh
points for our spatial discretization times the number of variables,
and a matrix $A$ which represents how all the entries of $\mathcal{X}$
are coupled to each other in the algebraic form of the equations:
$A\mathcal{X}(t_{k}) = \mathcal{X}(t_{k+1})$.

To solve this we could consider two approaches:
- *Explicit methods*: in this case $\mathcal{X}(t_{k+1})$ is expressed as a
  function of $\mathcal{X}(t_{k})$. This however will require resolving
  the /local/ dynamics (and be limited by the sound crossing time),
  making it impossible to perform a calculation for the /evolution/ of
  the star
- *Implicit methods:* in this case we write a function F(\mathcal{X}(t_{k}), \mathcal{X}(t_{k+1}))
  = 0 (e.g., F(\mathcal{X}(t_{k}), \mathcal{X}(t_{k+1})) \equiv A\mathcal{X}(t_{k}) - \mathcal{X}(t_{k+1}) = 0), and by solving the homogeneous
  equation we have obtained we derive \mathcal{X}(t_{k+1}). Implicit methods are
...
field, see this [[https://repository.aip.org/kippenhahn-rudolf-1978-june-18][1978 interview to Rudolf Kippenhahn]].
...
were women, and their contribution has been under-appreciated also (in
part) because of the predominant culture of considering algorithms as
technicalities.
...
about \sim10-100 CPUh per model.
...
Below is an (incomplete, biased) list of some of these codes, see also
guest lecture by Dr. Tom Wagg on <2026-03-06 Fri>.

** Semi-analytic approach to population synthesis

 - BSE: the forefather of many, see [[https://ui.adsabs.harvard.edu/abs/2002MNRAS.329..897H/abstract][Hurley 2002]] and references therein
   (*N.B.:* not /all/ rapid population synthesis codes derive from this,
   but many!)
...
* Homework: preparation for in class activity
 Next week, we are going to do a class activity to piece
 ...
 three masses (randomly assigned, one "low", "intermediate", and
 "high" ) to run with =MESA-web= (or your own MESA installation, see
 [[https://github.com/mathren/stellar_phys_545/tree/main/MESA_files/modernized_MESA_web][here]] for a setup based on =MESA-web= running with a recent MESA
 version). You will need to download and unzip on your laptop and
 have it ready by <2026-03-04 Wed>. I strongly suggest a quick visual
 inspection of the results before hand.
 ...
 *N.B.:* if the models below do not reach the desired stopping
 ...
 which control the temporal and spatial discretization, lower means
 higher resolution for both). Please keep the first model around too
 for the in class activity. Feel also free to change mass values and
 gather more stars if you want.
