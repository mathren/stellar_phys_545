...
of the name comes from "vis" which is latin for "forces".
...
We take the /first-order spatial moment of this equation/, which
...
particles to be constant) half the second time derivative of the
...
This is the general form of the Virial theorem derived by [[https://en.wikipedia.org/wiki/Rudolf_Clausius][Clausius]]. In
...
explicit we obtain: $E_\mathrm{kin} = |E_\mathrm{pot}|/2$. Since Keplerian orbits in
...
and rewrite the above as $3(\Gamma-1)E_\mathrm{int}$, and thus
