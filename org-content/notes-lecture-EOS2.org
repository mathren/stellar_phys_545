...
astrophysical cases. Moreover, the [[./notes-lecture-VirTheo.org][virial theorem]] describes the
gravothermal collapse of a collection of particles interacting through
long-range forces (e.g., the /self/-gravitating gas of a star) which
goes on until either (i) an energy source intervenes to cancel out the
surface energy losses to photons (and neutrinos), or (ii) the EOS
...
p\leq p_{F}  \ \  \mathrm{otherwise} \ \ 0 \ \ ,
  ...
  classical ideal gas (cf. Eq. 12 in [[file:notes-lecture-EOS1.org::*Ideal gas][the Ideal gas section]] using
  $n(p)=g_{e}(p)$), we just need the appropriate p\equiv p(\varepsilon) relaton

  *N.B.:* the functional dependence of the pressure $P$ on the
  (electron) density $n_{e}$ and ultimately $\rho$ comes from the Fermi
  momentum which appears in the extreme of the integral in $dp$.
...
/Ionization is the process of unbinding of an electron from an ion/, which
...
- Using the virial theorem, discuss which pressure term is more
  important in the total pressure as a function of the mass of the
  stars.
