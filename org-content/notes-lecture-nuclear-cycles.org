...
- There are really two main cycles, the CN cycle and the NO (the
  figure shows also minor branching out points). The second involves
  oxygen, which has a higher $Z$ and slightly higher Coulomb barriers,
  so it kicks in at slightly higher temperatures, but typically a star
  sustaining itself through the CN cycle will also do the NO cycle,
  and they are often referred to jointly as CNO cycle.
...
:Question:
- *Q*: How did population III stars do this?
:end:

*** pp \rightarrow CNO transition

Because of the higher Coulomb barriers involved in the CNO cycle (i.e.
the higher charge of $^{12}\mathrm{C}$, $^{14}\mathrm{N}$, and
$^{16}\mathrm{O}$), it has a more sensitive temperature dependence:
...
asteroseismology, see also [[./notes-lecture-asteroseismology.org][future lecture]]), stellar lifetimes inferred
from population analyses, etc.

The initial mass $M$ at which the transition happens needs to be
determined with stellar models and is around $M_{pp \rightarrow CNO} \sim 1.1-1.3M_{\odot}$,
where the imprecision comes from the systematic uncertainties in
stellar models (in terms of input physics and algorithmic
representation of the processes in the code, that we will discuss in
more [[./notes-lecture-codes.org][later]]). To some extent, this is a physically motivated threshold
to /define/ what a "massive" star is (for example this is typical in
asteroseismology or exoplanet contexts), though other meaningful
threshold exist: the term "massive star" is typically context
dependent.
...
dwarfs, [[https://ui.adsabs.harvard.edu/abs/2020ApJ...902L..36F/abstract][Farmer et al. 2020]], [[https://ui.adsabs.harvard.edu/abs/2021ApJ...912L..31W/abstract][Woosley \& Heger 2021]], [[https://ui.adsabs.harvard.edu/abs/2022ApJ...924...39M/abstract][Mehta et al. 2022]]
and [[https://ui.adsabs.harvard.edu/abs/2022ApJ...937..112F/abstract][Farag et al. 2022]] on black hole masses and most recently claims --
still under review -- from gravitational-wave detections by [[https://ui.adsabs.harvard.edu/abs/2025arXiv250904151T/abstract][Tong et
al. 2025]], [[https://ui.adsabs.harvard.edu/abs/2025arXiv250904637A/abstract][Antonini et al. 2025]] and references therein. Finally, see
also [[https://ui.adsabs.harvard.edu/abs/2023ApJ...945...41S/abstract][Shen et al. 2023]] for recent lab measurements).
...
Finally, if a star has reached Oxygen burning, gravity will compress
its core until it burns all the way to the most bound nucleus
(iron/nickel): stopping somehow the gravothermal collapse at this
point would require extreme fine-tuning, and by now the core density
is so high that the gravothermal collapse (of the core) is driven by
neutrino emission (as we will discuss in the [[./notes-lecture-neutrinos.org][next lecture]]) rather than
photon losses at the surface.
...
$Q_{\rm H-burn}$/4\simeq6.6 MeV/nucleon for H burning!).
...
elements to form heavier "iron group nuclei", that is nuclei with
$50\lesssim A \lesssim 70$ (which are also photodisintegrated and
...
#+CAPTION: $\varepsilon_\mathrm{nuc} \equiv \varepsilon_\mathrm{nuc}(T)$ at fixed $\rho$ and metallicity $Z$ on a log-log plot for the burning cycles that cover \ge99% of the stellar lifetime (H and He core burning). Credits: Frank Timmes, sourced from [[https://cococubed.com/code_pages/burn_hydrogen.shtml][cocodubed.com]].
#+ATTR_HTML: :width 100%
...
that in the Sun we expect some CN burning at a sub-dominant level
(while NO may not be active), and indeed $\nu_{e}$ from the decay of $^{13}N$
have been experimentally detected (one can recognize them from their
spectrum), see [[https://www.nature.com/articles/s41586-020-2934-0][Borexino collaboration 2020]].

Because of the increasing Coulomb barrier with $Z$ and $A=Z+N$ of the
heavier fuels, each heavier element requires a higher $T$ to burn (to
have a non-zero tunneling probability), so it is more centralized in
mass and radius. Moreover, moving inward $\varepsilon_\mathrm{nuc}$ depends more
...
depleted. At high masses ($M\geq20M_{\odot}$), the inner layers start cooling
through neutrinos, which may take away enough energy to prevent
convection from occurring (*N.B.:* convection kicks in only if needed to
transport the energy flux). Conversely, the outer layers remain
unburned which allows for the so called "onion layer" structure of
stars:

#+CAPTION: Layer structure for an initially $15M_{\odot}$ star at the end of its nuclear burning lifetime in mass coordinate from [[https://lavinia.as.arizona.edu/~mrenzo/materials/Thesis/Renzo_MSc_thesis.pdf][Renzo 2015]]. Note the final total mass is /not/ 15M_{\odot} because stars this massive lose mass through radiatively driven winds.
#+ATTR_HTML: :width 100%
[[./images/onion.png]]
...
layer with the fuel reservoir.

At the outer edge of each shell of the "onion" there can be (and
typically there is) an off-center burning region, so caled "shell
burning". The outer layer of the He core/inner layer of the H envelope
(the exact boundary depends on the definition one adopts) is
sufficiently hot to burn the remaining fuel there, and this burning
sustains the outer layer above it. Because of the presence of burning
shells, the core burning only needs to sustain the matter inside the
shell, which slightly complicates the /gravothermal/ argument we have
used. Note also that the location of the shell is determined by the
core structure (i.e., where there is available fuel): the $T$ in the
shell is determined by the stellar structure too and the shell burning
does not need to self-regulate. Conversely, the stellar structure
above the shell will have to adapt to the energy output of the shell
itself!
...
as the Coulomb barriers increase, cf. "onion structure", and $L$ tends
to increase): each subsequent fuel produces less and less energy per
barion, thus the burning must be faster and faster to compensate for
the surface This. losses also makes these phases more rare to observe
...
#+CAPTION: example of durations of burning phases for a few massive stars from [[https://lavinia.as.arizona.edu/~mrenzo/materials/Thesis/Renzo_MSc_thesis.pdf][Renzo 2015]].
#+ATTR_HTML: :width 100%
