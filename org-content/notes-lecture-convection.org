...
Cantiello's talk at KITP in 2017]], review by [[https://ui.adsabs.harvard.edu/abs/2023Galax..11...75J/abstract][Joyce & Tayar 2023]], [[https://ui.adsabs.harvard.edu/abs/1973ApJ...181..429J/abstract][Joss
et al. 1973]]
...
Convection is at the boundary between a /dynamical/ and a /thermal/
instability: as we will see it kicks in if the temperature gradient is
steeper than some threshold condition (so thermal properties of the
gas), although it results in /local/ motion of gas/plasma (dynamics).
However, the resulting velocities are very small compared to thermal
velocities and the /net/ flux of matter is zero. This regime straddling
across /thermal/ and /dynamical/ scales makes convection particularly
challenging to model directly across stellar scales.

Finally, convection (much like radiative diffusion and conduction) is
/not/ unique to stars: the monsoon clouds in Tucson's summer are also
...
The domain represents the core of a main sequence star of $\sim3M_{\odot}$.
...
*N.B.:* /Gaia/ has been predicted to be able to see the motion of the
photocenter of Betelgeuse due to convection in its envelope, see
[[https://ui.adsabs.harvard.edu/abs/2022A%26A...661L...1C/abstract][Chiavassa et al. 2022]], which means /Gaia/ cannot provide accurate proper
motion for the nearest red supergiant to Earth!
...
fraction of $^{20}\mathrm{Ne}$, and is taken from [[http://www.fen.upc.edu/users/jjose/][J. Jordi's personal
webpage]] (one of the co-authors).

#+HTML: <video  controls width="800" height="600" src="./images/125M-ONe-3.mp4" </video>
...
#+CAPTION: Schematic representation of an instability. A generic quantity starts from an initial steady state, when the instability kicks in, it leads to an exponential growth, until the instability saturates to a new steady state. The assumed functional behavior is $\small Q = Q_{\rm initial}+(Q_{\rm final}-Q_{\rm initial}) (e^{\gamma t}-1)/(e^{\gamma (t_{\rm saturation}-t_{\rm onset})}-1)$ with $\gamma$ growth rate of the instability $\small \gamma\sim i\omega$ with $\small \omega^{2}<0$. See generating script [[https://github.com/mathren/stellar_phys_545/tree/main/scripts][here]].
#+ATTR_HTML: :width 100%
[[./images/instability_onset.png]]
...
\end{equation}
#+end_latex
with $\nabla_{\mu} = \partial\log(\mu)/\partial\log(P)$. Eq. \ref{eq:ledoux_crit} is usually
referred to as the Ledoux criterion ([[https://ui.adsabs.harvard.edu/abs/1947ApJ...105..305L/abstract][Ledoux 1947]]).
...
Naively, one may think that the Ledoux criterion is more physically
...
from the flame. Intuitively, it's like taking a picture (assuming
ergodicity, that is any snapshot in time is statistically equivalent
to any other), and then averaging across the horizontal cross section
of this flame to obtain an approximation to the time- and space-
averaged vertical flow of energy and temperature. With all the
limitations that this entails, MLT is a very successful theory that is
sufficient for /most/ stellar evolution applications since those
typically are concerned with timescales that are very long w.r.t. the
convective turnover timescale (i.e., in the fire analogy, very long
compared to the "flickering" of the flames).
...
criterion. Now $\Delta\rightarrow\ell$ does not need to be infinitesimal anymore, just
small compared to the size of the star to allow for a linear
approximation for the temperature changes.
...
d \log(P)/dr$, and assume that $T_\mathrm{bubble} \simeq T_{\rm env} \equiv T$,
...
and the convective velocity $v_{\rm conv}$.
...
This should be related to the kinetic energy of the convective flow,
since buoyancy is what drives the convective instability, and thus to
the average velocity (in space and time) $v_{\mathrm{conv}}$.

:Question:
- *Q*: before we even do this calculation, can you imagine an
  upper-limit for $v_\mathrm{conv}$ in the approximated picture we are
  developing? (*Hint*: we have assumed that any pressure imbalance
  between the bubble and the environment would be quickly be washed
  out)
:end:
...
Nevertheless, even a velocity of $v_\mathrm{conv} \simeq 10^{-4}
c_\mathrm{sound}$ is sufficient to mix material very efficiently over
the evolutionary timescales (*N.B.:* the sound crossing time of a star
is $\sim \tau_{ff} \ll$ evolutionary timescales by a factor smaller than 10^{-4}.
For the Sun it's hours/billions of years - for now we are using
geological evidence on Earth to estimate the age of the Sun).
Therefore, /convection not only carries energy flux, but can also mix
(very efficienly) the chemical composition/!
...
- See [[https://ui.adsabs.harvard.edu/abs/1973ApJ...181..429J/abstract][Joss et al. 1973]], and [[https://ui.adsabs.harvard.edu/abs/2013ApJS..208....4P/abstract][Paxton et al. 2013, section 7]] on
  inefficient envelope convection for layers with $L \gtrsim L_{\rm
  Edd}$.
