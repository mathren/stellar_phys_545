...
#+CAPTION: Schematic representation of an instability. A generic quantity starts from an initial steady state, when the instability kicks in, it leads to an exponential growth, until the instability saturates to a new steady state. The assumed functional behavior is $Q = Q_{\rm initial}+(Q_{\rm final}-Q_{\rm initial}) (e^{\gamma t}-1)/(e^{\gamma (t_{\rm saturation}-t_{\rm onset})}-1)$.
#+ATTR_HTML: :width 50%
