#+title: ASTR545 - EOS (polytropic & ideal gas)
...
$\Delta T\simeq 10^{-4} K \ll T_{\rm surf} \ll T_{\rm center}$.
...
and other than these interactions between particles, they are free to
...
\end{equation}
#+end_latex
where we integrated over $\theta$. This gives the relationship between the
pressure $P$ and the distribution of particles in momentum space $p$.
Combining Eq. \ref{eq:P_statistic} with Eq. \ref{eq:Maxwellian} gives
the pressure.
...
By dimensional analysis, the pressure is a force per unit area, that
is an energy density:

#+BEGIN_latex
$$ [P] = [F]/L^{2} = ML/t^{2} \times 1/L^{2} = M L^{2}/t^{2} \times 1/L^{3} = [E]/L^{3}$$
#+END_latex

Therefore, Eq. \ref{eq:P_statistic} can be used to relate $P$ to the
internal energy density of a gas. This can be defined as:
...
temperature of the Sun in the [[./notes-lecture-VirTheo.org][next lecture]]). We also see that at
