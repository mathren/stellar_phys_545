#+title: ASTR545 - Diffusive energy transport
...
on timescales comparable to the dynamical timescale $\tau_{\rm ff}$.
...
*N.B.:* in this one-dimensional derivation the number density $n$ is per
unit length ([$n$] = 1/L).
...
of the density $n(z)$ using the assumption that the mean free path \ell
is small compared to the scale of interest! Eq. \ref{eq:diff} is the
generalization in 3 dimensions of the above, where $\nabla = (\partial_{x}, \partial_{y}, \partial_{z})$
for Cartesian coordinates, and $D=v\ell/3$.
...
Things are different though for degenerate gas (so inside
...
In the absence of convection (which we will treat [[./notes-lecture-convection.org][later]]) and neutrinos
...
instead are input physics for the star. We will [[./notes-lecture-nuclear-cycles.org][later]] unpack more
...
it can occasionally occur that $L_\mathrm{Edd} \le L$. In this case
