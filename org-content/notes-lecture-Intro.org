...
at the foundations of computational astrophysics since the early
1960s. By the 1980s-90s, stellar evolution was considered a "solved
problem": this was largely because of the large number of parameters
in (single) star models, which would allow one to fit any observable.
...
SN1987A whose progenitor was a blue supergiant star (while single star
evolution only predicts red supergiants and possibly H-free stars).

The advent of /Gaia/ space-based astrometry, the direct detection of GW,
the expanded capabilities (in timing and depth) of time-domain surveys
and precision of photometry (allowing for asteroseismology of large
samples) with KEPLER first, TESS now, and PLATO in the future have
ushered a real renaissance of this field, providing data and
theoretical questions that require deeper understanding of stellar
physics. Together with the development of *open-source* and
*community-driven* tools to address these questions, this has revived
significantly the field of stellar evolution, so much so that the
[[https://www.nationalacademies.org/projects/DEPS-BPA-18-01][Astro2020 decadal survey]] identified understanding stellar evolution
and interactions as a key priority! In this course we will cover the
basics needed to contribute to this efforts, and in the second half,
you will review some topics for which active research is ongoing.
