#+Title: ASTR545 - Mass transfer
#+author: Mathieu Renzo
#+email: mrenzo@arizona.edu
#+PREVIOUS_PAGE: notes-lecture-tides.org
#+NEXT_PAGE: notes-lecture-CE.org
...
We have already seen in the [[./notes-lecture-BIN.org::*Binaries are common][third lecture]] that, in the Universe,
...
Any exoplanet orbiting a host star is also a binary system from the
dynamical point of view, and several effects occurring in stellar
binaries (e.g., irradiation from a companion, tides, secular evolution
of the orbit, etc.) apply similarly to star-planets systems, albeit
typically with much more unequal mass-ratios between the orbiting
bodies.

Often enough, stars in binaries are so close to each other
than as they evolve, they will "touch" each other and exchange mass.

For O-type stars, we expect between 50-70% to experience some form of
binary mass transfer before their final collapse (regardless of their
metallicity at least down to SMC values, [[https://ui.adsabs.harvard.edu/abs/2025NatAs...9.1337S/abstract][Sana et al. 2025]]), implying
that binary interactions are the rule rather than the exception for
massive stars (e.g., [[https://ui.adsabs.harvard.edu/abs/2012Sci...337..444S/abstract][Sana et al. 2012]], [[https://ui.adsabs.harvard.edu/abs/2013A%26A...550A.107S/abstract][Sana et al. 2013]], but
references go back to [[https://ui.adsabs.harvard.edu/abs/1983ARA%26A..21..343A/abstract][Abt 1983]] and earlier all the way to [[https://ui.adsabs.harvard.edu/abs/1767RSPT...57..234M/abstract][Mitchell 1767]]!).
...
:Question:
- *Q*: What happens to the stellar radius as a typical star evolves?
...
being developed (e.g., [[https://ui.adsabs.harvard.edu/abs/2009ApJ...702.1387S/abstract][Sepinsky et al. 2009]], [[https://ui.adsabs.harvard.edu/abs/2025ApJ...983...39R/abstract][Rocha et al. 2025]]).

The second assumption is not critical, since the density increases
steeply moving inwards in the star: /most of the mass is in the center/.

The last assumption is typically easily verified as long as one
considers small parcels of gas.
...
On the line connecting the two binaries, $d\Phi/dr$ has three zeros,
meaning points where there is no net (gravitational+centrifugal)
force. All three are /saddle points/ and they are called Lagrangian
points:
...
centrifugal forces). The two don't coincide in general.
...
there is complete /tidal synchronization/ (see [[./notes-lecture-tides.org][previous lecture]]) In that
case, in the co-rotating frame, the gas of both stars is still,
$\mathbf{v}=0$, and the momentum conservation (Eq. \ref{eq:momentum}) reduces to
...
these surfaces: equipotential, isobaric and iso-density surfaces coincide!
...
Stars tend to grow bigger (in radius) as they evolve: even a binary system that
stars as detached may come into contact as the stars evolve,
triggering the onset of mass transfer between the stars in the binary
system. In fact, gas reaching L_{1} will be equally bound to either
stars, and it can easily be perturbed to fall into the other star (see
also review by [[https://ui.adsabs.harvard.edu/abs/2024ARA%26A..62...21M/abstract][Marchant & Bodensteiner 2024]]).
...
If Roche lobe overflow does not cause a runaway response of the orbit
(resulting in more and more overflow), then it is dynamically stable.

This can have important consequences for /both/ stars that will be modified
...
ultimately in a /common envelope event/ (see [[./notes-lecture-CE.org][next lecture]]).

Another case where a binary interaction may be dynamically unstable is
when the presence of a third companion drives the interaction, for
example in a triple system. In this case, the instability is triggered
by the orbital dynamics, and it may occur even in initially stable
systems as the stars evolve and lose mass through winds, expand in
radius (implying a larger impact of tides, see [[./notes-lecture-tides.org][previous lecture]]).
...
categories (defined by [[https://ui.adsabs.harvard.edu/abs/1967ZA.....66...58K/abstract][Kippenhahn et al. 1967]] and [[https://ui.adsabs.harvard.edu/abs/1970A%26A.....7..150L/abstract][Lauterborn 1970]]):
 ...
 - case B RLOF: the donor has a helium core (possibly inert and
   ...
   faster mass-transfer, although this may also depend on the metallicity and
...
*N.B.:* To remember this nomenclature, always think of the evolutionary
phase of the donor star at the onset of mass-transfer!
...
\end{equation}
#+end_latex
where \partial_{x}\Phi(L_{1})=0 along the line connecting the stars, and using the
Roche potential plus Kepler's 3^{rd} law this gives:
#+begin_latex
\begin{equation}
...
equilibrium themselves), cf. [[./notes-lecture-codes.org::*Population synthesis][notes on stellar evolution codes]].
...
Sometimes mass-transfer becomes dynamically unstable (see [[./notes-lecture-CE.org][see also
next lecture]]), and will ultimately result in the merger of two
stars. This process is quite common, about 20-40% of *massive* ($M_{\rm
ZAMS}\ge 7.5M_{\odot}$) *binaries* will merge, depending on assumptions on the
initial orbital distributions and treatment of mass transfer ([[https://ui.adsabs.harvard.edu/abs/2019A%26A...624A..66R/abstract][Renzo et
al. 2019b]]). The zeroth order effect of the presence of more
companions in a triple(, quadruple, etc...) is to enhance the number
of binary interactions (and also mergers) compared to a population of
