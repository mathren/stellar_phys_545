...
\end{equation}
#+end_latex
where $\nabla =\partial \log(T)/\partial \log(P)$ is the local temperature gradient,
equal to the radiative gradient in stably stratified regions:
#+begin_latex
\begin{equation}
...
the Sun this would go on for $\tau_{\rm KH}\simeq 15$ Myrs before it runs out of all of
...
also implies that the source of energy providing $\varepsilon_{\rm nuc}$ in a star
is a /self-regulating/ process: because nuclear reactions are a
consequence of the energy losses, during long-lived equilibrium phases
of the stellar life, they produce just enough energy to balance the
losses! In other words, $L_\mathrm{nuc} = L ( + L_{\nu})$.
...
the electron (which is acceptable since we are dealing with 10 MeV =
10\times 10^{6}eV as the relevant nuclear scale).
...
be resonant or not depending on the relative energy of the incoming
particles (the distinction will come back later).
...
for their antiparticles positron $e^{+}$,
positive muon
$\mu^{+}$,
and positive
...
$\rho_{n} \simeq Am_{u} / (4\pi/3 \times r_{n}^{3}) \simeq 2\times10^{14}$ g cm^{-3}, where $A$ cancels out.
...
Finally, actually carrying out the calculation give a probability of
tunneling ([[https://ui.adsabs.harvard.edu/abs/1928ZPhy...51..204G/abstract][Gamow 1928]])
#+begin_latex
\begin{equation}
...
possible via quantum tunneling through the Coulomb barrier, to
