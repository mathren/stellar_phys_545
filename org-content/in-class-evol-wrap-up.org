#+Title: ASTR545 - Qualitative evolution
#+author: Mathieu Renzo
#+email: mrenzo@arizona.edu
#+PREVIOUS_PAGE: notes-in-class-evol.org
#+NEXT_PAGE: notes-lecture-asteroseismology.org
...
1. *The EOS changes*, either because of phase transition from gas to
   liquid, and later solid (crystallization) in the formation of a
   rocky planet, or because into quantum mechanical effects (electron
   degeneracy) to *provide extra pressure*: this should not happen to
   any of your models, but it is how you form a *Brown Dwarfs* (BD) and
   a planets. The distinction between these is whether some minor
   nuclear burning, energetically unimportant but still affecting the
   composition happens before the object sets on gravothermal
   equilibrium: BD experience deuterium burning, but deuterium is a
   loosely bound nucleus and its burning doesn't release a lot of
   energy.
...
the ZAMS structure is fully determined by the composition and total
mass! This is why, although the star formation process does not
proceed starting with a ball of known composition $\{X_{i}\}$ and fixed
total mass $M$, we can make stellar models starting from this
idealized conditions, and let them relax to a thermal equilibrium
status solving the structure equations.

** Pre-main sequence stellar structure

#+CAPTION: Examples of pre-main sequence evolution along the Hayashi track for stellar models of varying metallicity. Credits: S. Cassisi.
#+ATTR_HTML: :width 50%
[[./images/Hayashi_composition.png]]
...
*N.B.:* The "beginning of the numerical simulation" here represents the
moment when the proto-stellar core, which has stopped contracting
isothermally because $\rho$ has risen enough that $\kappa$ in the IR
wavelength region is high, and transition to an adiabatic contraction,
also stops accreting mass ($\dot{M}=0$) and reached the total initial
mass $M$ that we initialize the runs with.
...
obtain enough tunneling through the Coulomb barriers such that the
nuclear reaction energy generation rate is sufficient to release
enough energy to compensate for the surface losses.
...
$\kappa\simeq\kappa_\mathrm{es}$ is progressively better since fewer bound-bound and
...
partially degenerate - thus sustaining itself against gravitational
...
** "Low mass" star with radiative main-sequence cores
...
*** He ignition in degenerate conditions: flash
...
for cosmological applications (see e.g., [[https://ui.adsabs.harvard.edu/abs/1993ApJ...417..553L/abstract][Lee et al. 1993]]).
...
#+CAPTION: /Gaia/ color-magnitude diagrams for various globular clusters of varying chemical composition indicated by [Fe/H]. The red clump and horizontal branch are visible at $M_{G}\sim1$ and spanning a range of colors (i.e., $T_{\rm eff}$). This is Fig. 3 from [[https://ui.adsabs.harvard.edu/abs/2018A%2526A...616A..10G/abstrac][Gaia collaboration 2018]].
#+ATTR_HTML: :width 100%
[[./images/Gaia_HRD_GC.png]]
...
#+CAPTION: /Gaia/ color-magnitude diagrams for the open cluster NGC7419, showing a gap between the colors of the main sequence and red supergiants. This is Fig. 4 from [[https://ui.adsabs.harvard.edu/abs/2025MNRAS.541.1866C/abstract][Chakrabhorty et al. 2025]].
#+ATTR_HTML: :width 100%
[[./images/NGC7419.png]]
...
#+CAPTION: Luminosity (colors) in nuclear, thermal and total neutrinos and "compactness" metric for explodability (dotted, right y-axis, see also [[./notes-lecture-SNe.org][lecture on core-collapse supernovae]]) for a 15M_{\odot}. The x-axis is used to emphasize the late phases: spikes in nuclear neutrinos correspond to Si core and shell ignition in partially degenerate environments, that is "silicon flashes". This is Fig. 7 in [[https://ui.adsabs.harvard.edu/abs/2017A%26A...603A.118R/abstract][Renzo et al. 2017]].
#+ATTR_HTML: :width 100%
[[./images/Lnu15_xi.png]]
...
*** super-AGB stars and off-center C ignition
