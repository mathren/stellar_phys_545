...
;; ;; over-ride html ids in memory to not clutter *.org content
;; (defun mr/slugify (text)
;;   "Convert TEXT to a URL-friendly slug."
;;   (downcase
;;    (replace-regexp-in-string
...
;; (defvar mr/id-map (make-hash-table :test 'equal)
;;   "Map from random org IDs to deterministic slugs.")

;; (defun mr/fix-remaining-ids (output backend info)
;;   "Replace all remaining random org IDs in final HTML output."
;;   (when (org-export-derived-backend-p backend 'html)
;;     (replace-regexp-in-string
;;      "org[a-f0-9]\\{7\\}"
;;      (lambda (match)
;;        (or (gethash match mr/id-map) match))
;;      output)))
